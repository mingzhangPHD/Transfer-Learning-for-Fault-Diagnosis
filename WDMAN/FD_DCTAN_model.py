...
def load_data(data_path, classes = 4, x_type = "X"):
...
class WDMAN(object):
    ...
    def __init__(self, sess,
        ...
        self.sess = sess
        self.crop = crop
        self.model_name = "WDMAN" + '_' + x_type
        self.dataset_name_s = dataset_name_s
        self.dataset_name_t = dataset_name_t

        self.save_path = 'results/' + self.model_name + '/' + self.dataset_name_s + '_' + self.dataset_name_t[-17:]
        if not os.path.exists(self.save_path):
            os.makedirs(self.save_path)

        self.checkpoint_dir = checkpoint_dir + '/' + self.model_name + '/' + self.dataset_name_s + '_' + self.dataset_name_t[-17:]
        if not os.path.exists(self.checkpoint_dir):
            os.makedirs(self.checkpoint_dir)

        self.log_dir = 'log/' + self.model_name + '/' + self.dataset_name_s + '_' + self.dataset_name_t[-17:]
        ...
        self.base_dir = checkpoint_dir + '/' + self.base_model + '/' + self.dataset_name_s + '_' + self.dataset_name_t[-17:]
        ...
        self.trainThre = 0.94
        self.testThre = 0.94
        self.trainThre_pre = 0.99
        self.testThre_pre = 0.99
    ...
    def build_model(self):
        ...
        with tf.variable_scope("NETs"):
            ...
            s_temp_c01 = self.s_list[0]
            t_temp_c01 = self.t_list[0]
            ...
            s_temp_c02 = self.s_list[1]
            t_temp_c02 = self.t_list[1]
    ...
    def train_01(self, config):
        ...
        tf.set_random_seed(1)  # to keep consistent results
        c_optim = tf.train.AdamOptimizer(config.learning_rate/5) \
            .minimize(self.c_loss, var_list=self.fs_vars + self.fc_vars + self.c_vars)
        c_optim_fine = tf.train.AdamOptimizer(config.learning_rate/10) \
            .minimize(self.c_loss, var_list=self.fs_vars + self.fc_vars + self.c_vars)
        ...
        for epoch in range(config.epoch):
            ...
            if sign_Classifier:
                _, cost = self.sess.run([c_optim_fine, self.c_loss],
                                        feed_dict={self.X_s: minibatch_X_s, self.Y_s: minibatch_Y_s,
                                                   self.X_t: minibatch_X_t, self.Y_t: minibatch_Y_t})
    ...
    def feature_s(self, x, y=None, reuse=False, is_train=True, with_sum=True):

        return self.feature_s_base(x, y, reuse=reuse, is_train=is_train, with_sum=with_sum)

    def feature_s_base(self, x, y=None, reuse=False, is_train=True, with_sum=True):

        with tf.variable_scope("feature_source") as scope:
            if reuse:
                scope.reuse_variables()
            x_in = tf.reshape(x, [-1, x.get_shape().as_list()[1], 1])
            h0, w0, b0 = conv1d(x_in, self.ff_dim/2, name='fs_h0_conv', with_w=True)
            # h0 = tf.nn.tanh(h0)
            h0 = tf.nn.relu(h0)
            # h0 = lrelu(h0)
            output = max_pool1d(h0, name='fs_h0_pool')

            h1, w1, b1 = conv1d(output, self.ff_dim, name='fs_h1_conv', with_w=True)
            # h1 = tf.nn.tanh(h1)
            h1 = tf.nn.relu(h1)
            # h1 = lrelu(h1)
            output = max_pool1d(h1, name='fs_h1_pool')

            h2, w2, b2 = conv1d(output, self.ff_dim * 2, name='fs_h2_conv', with_w=True)
            h2 = tf.nn.relu(h2)
            ...
            feature_sum_s = [h0_sum, h1_sum,h2_sum]
    ...
    def feature_t(self, x, y=None, reuse=False, is_train=True, with_sum=True):

        return self.feature_t_base(x, y, reuse=reuse, is_train=is_train, with_sum=with_sum)

    def feature_t_base(self, x, y=None, reuse=False, is_train=True, with_sum=True):

        with tf.variable_scope("feature_target") as scope:
            if reuse:
                scope.reuse_variables()
            x_in = tf.reshape(x, [-1, x.get_shape().as_list()[1], 1])
            h0, w0, b0 = conv1d(x_in, self.ff_dim/2, name='ft_h0_conv', with_w=True)
            # h0 = tf.nn.tanh(h0)
            h0 = tf.nn.relu(h0)
            # h0 = lrelu(h0)
            h0 = max_pool1d(h0, name='ft_h0_pool')

            h1, w1, b1 = conv1d(h0, self.ff_dim, name='ft_h1_conv', with_w=True)
            # h1 = tf.nn.tanh(h1)
            h1 = tf.nn.relu(h1)
            # h1 = lrelu(h1)
            h1 = max_pool1d(h1, name='ft_h1_pool')

            # h2, w2, b2 = conv1d(h1, self.ff_dim * 4, name='fs_h2_conv', with_w=True)
            # h2 = tf.nn.relu(h2)
            # h2 = max_pool1d(h2, name='fs_h2_pool')
            #
            # h3, w3, b3 = conv1d(h2, self.ff_dim * 4, name='fs_h3_conv', with_w=True)
            # h3 = tf.nn.tanh(h3)
            # h3 = max_pool1d(h3, name='fs_h3_pool')

            output = h1
            h0_sum = histogram_summary("ft_h0", h0)
            h1_sum = histogram_summary("ft_h1", h1)
            # h2_sum = histogram_summary("fs_h2", h2)

            feature_sum_t = [h0_sum, h1_sum]

            if with_sum:
                return output, feature_sum_t
            else:
                return output
    ...
    def classifier_base(self, f, y=None, reuse=False, is_train=True, with_sum=True, with_list=False):

        with tf.variable_scope("classifier") as scope:
            if reuse:
                scope.reuse_variables()

            h1 = fully_conn(f, self.ffc_dim/2, name='c_h1_conn')
            h1 = tf.nn.relu(h1)
            h3 = linear(h1, self.y_dim, scope='c_h3_lin')


            h1_sum = histogram_summary("c_h1", h1)
            h3_sum = histogram_summary("c_h3", h3)

            classifier_sum = [h1_sum, h3_sum]

            h_list = [h1, tf.nn.softmax(h3)]
            if with_sum and not with_list:
                return tf.nn.softmax(h3), h3, classifier_sum
            elif with_list and not with_sum:
                return tf.nn.softmax(h3), h3, h_list
            elif with_sum and with_list:
                return tf.nn.softmax(h3), h3, classifier_sum, h_list
            else:
    ...
    def discriminator_f_base(self, f, y=None, reuse=False, is_train=True, with_sum=True):
        ...
        with tf.variable_scope("discriminator_f") as scope:
            ...
            h2, w2, b2 = fully_conn(h0, self.dfc_dim/1, name='df_h2_conn', with_w=True)
            h2 = lrelu(h2)
    ...
    def discriminator_c01_base(self, f, y=None, reuse=False, is_train=True, with_sum=True):
        ...
        with tf.variable_scope("discriminator_c01") as scope:
            ...
            # h0, w0, b0 = conv1d(f, self.df_dim * 4, name='dc01_h0_conv', with_w=True)
            # # h0         = tf.nn.relu(h0)
            # h0 = lrelu(h0)
    ...
    def discriminator_c02_base(self, f, y=None, reuse=False, is_train=True, with_sum=True):
        ...
        with tf.variable_scope("discriminator_c02") as scope:
            ...
            # h0, w0, b0 = conv1d(f, self.df_dim * 4, name='dc02_h0_conv', with_w=True)
            # # h0         = tf.nn.relu(h0)
            # h0 = lrelu(h0)
            # # h0 = max_pool1d(h0, name='d_h0_pool')
    ...
    def predictor_s(self, x, y=None):
        ...
        h_list = F_sc_list + C_list
        return P, P_logits, h_list

    def predictor_t(self, x, y=None):

        F_t = self.feature_t(x, y, reuse=True, is_train=False, with_sum=False)
        F_tc, F_tc_list = self.feature_c(F_t, y, reuse=True, is_train=False, with_sum=False, with_list=True)
        P, P_logits, C_list = self.classifier(F_tc, y, reuse=True, is_train=False, with_sum=False, with_list=True)
        h_list = F_tc_list + C_list
    ...
    def print_accuracy_s(self, data):
        ...
        if self.epoch % self.epochWriteThre == 0:
            print(" Source Train Accuary = { %s : %s, %s, %s, %s}"
                % (r, r_c[0], r_c[1], r_c[2], r_c[3]))
        r_train = r
        r_c_train = r_c
        ...
        if self.epoch % self.epochWriteThre == 0:
            print(" Source Test Accuary = { %s : %s, %s, %s, %s}"
                % (r, r_c[0], r_c[1], r_c[2], r_c[3]))
        r_test = r
        r_c_test = r_c
        ...
        if self.epoch % self.epochWriteThre == 0:
            print(" Source      Accuary = { %s : %s, %s, %s, %s}"
                % (r, r_c[0], r_c[1], r_c[2], r_c[3]))
    ...
    def print_accuracy_t(self, data):
        ...
        if self.epoch % self.epochWriteThre == 0:
            print(" Target Train Accuary = { %s : %s, %s, %s, %s}"
                % (r, r_c[0], r_c[1], r_c[2], r_c[3]))
        r_train = r
        r_c_train = r_c
        ...
        if self.epoch % self.epochWriteThre == 0:
            ...
            print(" Target Test Accuary = { %s : %s, %s, %s, %s}"
                % (r, r_c[0], r_c[1], r_c[2], r_c[3]))
        r_test = r
        r_c_test = r_c
        ...
        if self.epoch % self.epochWriteThre == 0:
            ...
            print(" Target      Accuary = { %s : %s, %s, %s, %s}"
                % (r, r_c[0], r_c[1], r_c[2], r_c[3]))
